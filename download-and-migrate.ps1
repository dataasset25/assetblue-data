...
# This script works without Git installed

Write-Host "=== Repository Migration Script (No Git Required) ===" -ForegroundColor Green
Write-Host ""
...
Write-Host "  1. Download each repository as ZIP from GitHub"
Write-Host "  2. Extract files into respective folders"
...
foreach ($repo in $repos) {
    ...
    try {
        ...
        if ($extractedFolder) {
            ...
            Write-Host "  Successfully migrated $($repo.Name)" -ForegroundColor Green
        } else {
...
Write-Host "  2. Install Git from https://git-scm.com/download/win (if not installed)"
